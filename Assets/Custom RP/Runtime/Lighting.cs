...
public class Lighting
{
    ...
    private static int dirLightCountId = Shader.PropertyToID("_DirectionalLightCount"),
        ...
        dirLightDirectionId = Shader.PropertyToID("_DirectionalLightDirectionsAndMasks"),
    ...
    private static Vector4[] otherLightColors = new Vector4[maxOtherLightCount],
        ...
        otherLightDirectionsAndMasks = new Vector4[maxOtherLightCount], //聚光灯朝向
    ...
    public void Setup(ScriptableRenderContext context, CullingResults cullingResults, ShadowSettings shadowSettings, bool useLightsPerObject, int renderingLayerMask)
    {
        ...
        SetupLights(useLightsPerObject, renderingLayerMask); //将各类灯光数据传递给shader
    ...
    void SetupLights(bool useLightsPerObject, int renderingLayerMask)
    {
        ...
        if (dirLightCount > 0)
        {
            ...
            buffer.SetGlobalVectorArray(dirLightDirectionId, dirLightDirectionsAndMasks);   // 传递光源的方向
        ...
        if (otherLightCount > 0)
        {
            ...
            buffer.SetGlobalVectorArray(otherLightDirectionsId, otherLightDirectionsAndMasks); //传递聚光灯的朝向
    ...
    /// </summary>
    void SetupDirectionalLight(int index, int visibleIndex, ref VisibleLight visibleLight, Light light) //消耗大 使用ref引用传递函数
    {
        dirLightColors[index] = visibleLight.finalColor; //光源颜色乘以强度 但默认不会将其转为线性空间 得手动设置
        Vector4 dirAndMask = -visibleLight.localToWorldMatrix.GetColumn(2); //光源变换矩阵
        dirAndMask.w = light.renderingLayerMask.ReinterpretAsFloat();
        dirLightDirectionsAndMasks[index] = dirAndMask;
        dirLightShadowData[index] = shadows.ReserveDirectionalShadow(light, visibleIndex); //阴影的剔除相关 外加阴影强度和索引
    }
    ...
    void SetupSpotLight(int index, int visibleIndex, ref VisibleLight visibleLight, Light light)
    {
        ...
        otherLightPositions[index] = position;                                              // 位置获取
        Vector4 dirAndMask = -visibleLight.localToWorldMatrix.GetColumn(2); //聚光灯朝向获取
        dirAndMask.w = light.renderingLayerMask.ReinterpretAsFloat();
        otherLightDirectionsAndMasks[index] = dirAndMask;
